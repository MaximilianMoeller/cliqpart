...
vector<unique_ptr<IAbstractSeparator>> SeparatorFactory::BuildSeparator(const RunConfig &config, ModelWrapper &data) {
  ...
  for (ConfigVariant sep_config : config.separator_configs) {
	if (holds_alternative<TriangleSeparatorConfig>(sep_config)) {
	  ...
	  res.emplace_back(make_unique<TriangleSeparator>(data, get<TriangleSeparatorConfig>(sep_config)));
	} else if (holds_alternative<StSeparatorConfig>(sep_config)) {
	  PLOGD << "Creating new [S:T] separator.";
	  res.emplace_back(make_unique<StSeparator>(data, get<StSeparatorConfig>(sep_config)));
	}
