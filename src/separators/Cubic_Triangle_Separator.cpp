...
void Cubic_Triangle_Separator::callback() {
    try {
        if (where == GRB_CB_MIPSOL) {
            ...
            for (int i = 0; i < degree; ++i) {
                for (int j = 0; j < i; ++j) {
                    // if the edge is in the CP, i.e. x_ij=1, no triangle inequality can be violated -> continue
                    auto x_ij = vars[i][j];
                    ...
                    for (int k = 0; k < j; ++k) {
                        ...
                        // a triangle inequality is violated iff the sum of the other edges in the triangle is 2;
                        // i.e. edge ij is not in the clique partitioning while edges ik and jk are.
                        ...
                        if (1.75 < sum && sum < 2.25) {
                            ...
                            if (verbosity == Verbosity::Trace)
                                clog << "Δ(" << endl << "\t" << i << "-" << j << ":" << v_ij << endl << "\t" << i << "-"
                                     << k << ":" << v_ik << endl << "\t" << j << "-" << k << ":" << v_jk << endl << ")"
                                     << endl;
