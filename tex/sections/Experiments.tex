...
Firstly, while logging important variables (\eg, the number of cuts added and removed) every iteration, we made the mistake to also log every solution to the LP relaxation.
...
Secondly, we did not log the times it took to solve the LP relaxation and then separating it independently.
Rather we only recorded progress once every iteration and used the fact that the logging library timestamps every logged message to deduce running times.
We can therefore not confidently say whether the time spend in one iteration is to be attributed to the LP solver or our separation routines.
...
Especially in (but not limited to) the first few iterations, many violated constraints can be found quickly, leading to durations in the order of milliseconds for a whole iteration.
Nevertheless, we regret these flaws and suggest to interpret the obtained results with some caveats.

We will thus round all times recorded to 0.1 seconds and make explicitly clear when even less time was spend by writing $\leq 0.1$.
